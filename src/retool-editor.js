...
const RetoolEditor = ({ triggerQuery, model, modelUpdate }) => {
  ...
  const editor = useEditor({
    ...
    onUpdate: (event) => {
      modelUpdate({
        description: event.editor.getJSON(),
      });
    },
  });
  ...
  useEffect(() => {
    if (value != null) {
      ...
      const selection = editor?.view.state.selection.$anchor.pos;

      if (commands) {
        commands.setContent(value ?? DEFAULT_VALUE);
        if (selection) commands.setTextSelection(selection);
